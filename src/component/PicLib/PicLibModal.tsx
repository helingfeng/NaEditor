...
class PicLibModal extends React.Component<ContentProps, any> {
  ...
  renderTitle = () => {
    return [
      <span className="d-title" key={1}>
        图片选择器
      </span>,
      <span className="d-desc" key={2}>
        (支持格式为xxx)
      </span>,
    ];
  };
    ...
    return imgList.map((v: ImageInfo, i: number) => (
      <Col span={3} key={i}>
        ...
        >
          ...
          />
          <p className="d-image-name" title={v.name}>
            {v.name}
          </p>
    ...
    return (
      ...
      >
        <div className="d-content">
          ...
          <Row style={{ marginTop: '10px' }}>{this.renderImgList()}</Row>
        </div>
      </Modal>
    );
  }
