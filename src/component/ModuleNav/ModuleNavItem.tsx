...
class ModuleNavItem extends PureComponent<
  IModuleNavItemProps,
  IModuleNavItemState
> {
  static contextTypes = {
  ...
  onDrop = (e: React.DragEvent) => {
    const targetModuleId: number = Number.parseInt(
      e.dataTransfer.getData('moduleId'),
      10,
    );
    const {
      moduleData: { moduleId },
        ...
        >
          ...
          <span className="d-module-operate">
            ...
            <Icon
              ...
              type="delete"
              onClick={e => {
                removeModuleRequest({ moduleId, pageId });
              }}
            />
          </span>
        </li>
...
const mapStateToProps = (state: IState, props: any) => {
  ...
  const { isVisible: isConfigVisible } = state.moduleConfig;
