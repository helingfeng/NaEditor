...
class ManagePage extends React.Component<any, any> {
  ...
  viewUrl = (url: string) => {
    return (
      <div className="d-popover-wrap">
        <QRCode value={url} />
        <div className="d-copy-link">{url}</div>
      </div>
    );
  };
  ...
  render() {
    const columns = [
      {
        ...
        render: (item: IPage) => {
          return (
            <a
              href={`${INTERFACE.viewAddress}?pageId=${item.id}`}
              target="_blank"
            >
              {item.pageName}
            </a>
        ...
        render: (item: IPage) => {
          return (
            <React.Fragment>
              ...
              <Popover
                content={this.viewUrl(
                  `https:${INTERFACE.viewAddress}?pageId=${item.id}`,
                )}
                title="浏览地址"
                trigger="hover"
              >
    ...
    return (
      <div>
        <TopBar hasPreview={false} hasPublish={false} username={username} />
